...
def test_network_init_strings(subtests: pytest.Subtests):
    ...
    nx.add_edges_from((l, l.upper()) for l in ascii_lowercase)
...
def test_add_edge_create_nodes():
    ...
    assert 1 in nx and 2 in nx
    

def test_add_edges_create_nodes():
    nx = Network()
    nx.add_edges_from(
        zip(range(10), reversed(range(10))), create_nodes=True
    )
    assert len(nx.nodes) == 10
...
def test_getitem(subtests: pytest.Subtests):
    nx = Network(range(10), zip(range(10), reversed(range(10))))
